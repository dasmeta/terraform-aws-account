...
As an MSP platform operator, I need to enable weekly uptime and latency collection with a scoped ingress selector instead of copying long queries into every account configuration.
...
1. **Given** a valid application source and ingress metric scope, **When** the account configuration is evaluated, **Then** canonical uptime and latency queries are generated automatically.
...
1. **Given** application collection with neither a metric scope nor a complete query pair, **When** configuration is validated, **Then** it is rejected.
...
- **FR-001**: Application configuration MUST accept an optional ingress metric scope for canonical query generation.
- **FR-002**: When no explicit query pair is supplied, an enabled Prometheus application source MUST require a non-empty metric scope.
...
- **SC-004**: Invalid empty and partial configurations are rejected during planning rather than at collection time.
