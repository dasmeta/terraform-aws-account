...
| <a name="input_log_metrics"></a> [log\_metrics](#input\_log\_metrics) | Provide CloudWatch Log Metric filters | <pre>object({<br>    enabled = optional(bool, false)<br>    metrics_patterns = optional(list(object({<br>      name       = optional(string, "ERROR")<br>      pattern    = optional(string, "ERROR")<br>      unit       = optional(string, "None")<br>      dimensions = optional(map(string), {})<br>      })), [<br>      {<br>        name    = "Failed-login-attempts"<br>        pattern = "{ $.eventName = ConsoleLogin } && ($.errorMessage = *Failed*)"<br>      }<br>      ]<br>    )<br>    metrics_namespace = optional(string, "LogBasedMetrics")<br>  })</pre> | <pre>{<br>  "enabled": false<br>}</pre> | no |
